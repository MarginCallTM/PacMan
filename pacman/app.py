"""Shared implementation behind every launch style.

Deliberately not named ``__main__.py``: that name would make
``python -m pacman`` work automatically, which is not one of the
supported launch styles here. The two things that *are* supported --
``python3 pac-man.py config.json`` (the subject's launch contract)
and running the built wheel directly (``python pacman.whl
config.json``) -- both just import :func:`run` from this module, so
the real logic exists in exactly one place.
"""
import sys
...
def run() -> None:
    """Run :func:`main` under the top-level error guard, then exit.

    The single source of truth for "never a Python traceback": shared
    by both launch shims (``pac-man.py`` and the root ``__main__.py``
    used to run the built wheel), so the guard only has to be written
    and tested once.
    """
    try:
